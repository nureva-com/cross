#!/usr/bin/env bash
#
# The original `cross` implementation tries to be generic across a bunch of versions of Linux and architectures.
# Too much has changed in Ubuntu 24.04 to do this so this file likely only works for `arm64` and Ubuntu 24.04 so
# it likely won't work for any other combination. A lot of this extra stuff is removed.
...
main() {
    ...
    # This is specific to the Ubuntu 24.04 `noble` package repositories. We weren't able to get the debian repos to
    # work because there were pre-installed system packages (ncurses-base) already coming from Ubuntu repos while this
    # script tried to get an older version from the debian repos which filed.
    local arch="${1}" \
        kversion=6.8.0-31

    # These aren't used because we use the standard Ubuntu 24.04 repositories
